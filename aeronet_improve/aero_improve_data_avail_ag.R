...
improve.pm25 <- improve.dat[!is.na(improve.dat$pm25)]
...
improve.obs <- improve.climo.obs[,c(1:3,11,12)]
...
#get selected states
states.sel <- subset(states, region == "nebraska" | 
                       region == "kansas" | 
                       region == "south dakota" | 
                       region == "north dakota" |
                       region == "oklahoma" |
...
#map aeronet trends
p <- ggplot() + geom_polygon(data = states.sel, aes(x=long,y=lat, group = group), fill=NA, color="black") +
